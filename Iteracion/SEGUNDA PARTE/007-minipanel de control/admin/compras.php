...
if ($loadError === "" && $q !== "") {
  $compras = array_values(array_filter($compras, function($c) use ($qLower){
    ...
    $productName = (string)get_field($c, ["product_name","producto.nombre","producto.name","product.nombre","product.name"], "");
            ...
            <?php else: ?>
              <?php foreach ($compras as $c): ?>
                <?php
                  ...
                  $productName = (string)get_field($c, ["product_name","producto.nombre","producto.name","product.nombre","product.name"], "—");
                  $qty = (int)get_field($c, ["qty","producto.qty","product.qty"], 1);
                  $color = (string)get_field($c, ["color","producto.color","product.color"], "—");
                  $total = get_field($c, ["total","producto.total","product.total"], "");
                ?>
                <tr>
